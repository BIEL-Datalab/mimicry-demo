...
class ObjectTracker:
    ...
    def load_static_boxes(self) -> None:
        ...
        加载静态框配置
        行为:
          - 从 JSON 读取 Box_IN/OUT/NG 等静态区域
          - 更新 last_boxes 中的对应配置
        """
        try:
    ...
    def process_frame(self, img: np.ndarray) -> tuple[np.ndarray, bool]:
        ...
        with self.lock:
            ...
            if not self.in_cycle:
                for hand_box in hand_boxes:
                    if box_in is not None:
                        ...
                        # print(f"Debug: Hand-BoxIN IoM={iom:.2f}") # 调试用
            ...
            if self.in_cycle:
                # 检查手是否还在 Box_IN 区域内，更新 has_left_start_box 状态
                is_hand_in_box_in = False
                for hand_box in hand_boxes:
                    if (
                        box_in is not None
                        and self.compute_iom(hand_box, box_in) > self.hand_glass_threshold
                    ):
                        is_hand_in_box_in = True
                        break
                
                if not is_hand_in_box_in:
                    self.has_left_start_box = True

                end_cycle = False
                for hand_box in hand_boxes:
                    # 使用 Box_IN 作为结束判断基准，必须先离开过 Box_IN
                    if (
                        box_in is not None
                        and self.compute_iom(hand_box, box_in) > self.hand_glass_threshold
                        and self.has_left_start_box
                    ):
                        self.current_box = "OK" # 默认为OK，因为都在 Box_IN
                        end_cycle = True
                        break
                        
                    # 仍然保留 Box_NG 的判断，如果有的话
                    if (
                        box_ng is not None
                ...
                if end_cycle:
                    ...
                    if current_frames >= self.MIN_CYCLE_FRAMES:
                        # 确定结束时使用的是哪个box，方便打印
                        target_box = box_ng if self.current_box == "NG" else box_in
                        print(
                            f"Cycle End at miou: {self.compute_iom(hand_box, target_box)}"
                        )
                        # print(
                        #     "End time:",
                        #     time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
    ...
    def end_cycle(self) -> dict[str, float | int | str | None]:
        ...
        周期结束处理
        返回:
          周期统计字典，包括 id/时间戳/总时长/横纵向时长/玻璃结果 等
        关键流程:
          - 依据类别投票确定最终类别
          - 更新累计统计 OK/NG
          - 计算各姿态段的持续时间
...
if __name__ == "__main__":
    ...
    process_video_stream(
        video_path="videos/raw/pad-glass-BA远程13_L3-3F-313#照检查台_20260108085800_20260108110000.avi",
